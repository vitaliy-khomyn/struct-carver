...
class AVIParser(BaseFormatParser):
    engine_type = "binary"
    ...
    def clone(self) -> 'AVIParser':
        new_parser = AVIParser()
    ...
    def analyze_binary(self, data: bytes, bytes_remaining: int = 0) -> Tuple[bool, bool, int, int]:
        ...
        if not self.is_open:
            if not self.pending_header:
                # Search for RIFF followed by 'AVI ' at offset +8 to avoid matching WAV/WEBP chunks
                start_idx = -1
                search_from = 0
                while True:
                    ...
                    if riff_pos == -1:
                        break
                    if riff_pos + 12 <= n and data[riff_pos + 8 : riff_pos + 12] == b'AVI ':
                        start_idx = riff_pos
                        break
            ...
            if header_block[8:12] != b'AVI ':
                return True, False, 0, 0

            riff_size = struct.unpack('<I', header_block[4:8])[0]
            self.total_size = riff_size + 8

            if self.total_size < 12 or self.total_size > 10 * 1024 * 1024 * 1024:
                return True, False, 0, 0
