...
class AVIParser(BaseFormatParser):
    ...
    def analyze_binary(self, data: bytes, bytes_remaining: int = 0) -> Tuple[bool, bool, int, int]:
        ...
        if not self.is_open:
            ...
            if start_idx != -1:
                # Need at least 12 bytes to read size and 'AVI '
                if n - start_idx < 12:
                    return False, False, n, 12 - (n - start_idx)

                # Validate format 'AVI '
                if data[start_idx + 8 : start_idx + 12] != b'AVI ':
                ...
                # Safety check
                if self.total_size < 12 or self.total_size > 10 * 1024 * 1024 * 1024: # 10GB safety limit
                    return True, False, 0, 0
