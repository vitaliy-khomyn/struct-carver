...
class MOVParser(BaseFormatParser):
    ...
    def header_signatures(self) -> List[bytes]:
        # QuickTime MOV starts with ftyp box, moov box, or free box
        return [b'ftyp', b'moov', b'free']
    ...
    def analyze_binary(self, data: bytes, bytes_remaining: int = 0) -> Tuple[bool, bool, int, int]:
        ...
        if not self.is_open:
            # Look for ftyp or moov or free at box boundary (starts 4 bytes earlier as size)
            ftyp_idx = data.find(b'ftyp')
            moov_idx = data.find(b'moov')
            free_idx = data.find(b'free')
            valid_indices = [p for p in [ftyp_idx, moov_idx, free_idx] if p >= 4]
